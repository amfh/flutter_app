...
class PublicationService {
      ...
      if (value is String) {
        ...
        for (final match in matches) {
          ...
          if (url != null && url.isNotEmpty) {
            ...
            if (url.startsWith('/')) {
              final absoluteUrl = 'https://nye.kompetansebiblioteket.no$url';
    ...
    for (final imgRegex in regexPatterns) {
      processedHtml = processedHtml.replaceAllMapped(imgRegex, (match) {
        ...
        if (originalUrl.startsWith('/')) {
          absoluteUrl = 'https://nye.kompetansebiblioteket.no$originalUrl';
  ...
  Future<void> downloadAndCacheContentOnly(String publicationId) async {
    try {
      ...
      if (data != null) {
        ...
        print('💾 UpdateDate saved in content data for update checking');
  ...
  // Last ned og cache kun innhold (uten bilder) for en publikasjon med progress callback
  Future<void> downloadAndCacheContentOnlyWithProgress(
    String publicationId, {
    required Function(double progress, String status) onProgress,
  }) async {
    try {
      print(
          '🚀 STARTING downloadAndCacheContentOnlyWithProgress for ID: $publicationId');

      onProgress(0.1, 'Kobler til API...');
      print('Fetching full publication data for ID: $publicationId');

      final data = await fetchFullPublicationFromApiWithProgress(
        publicationId,
        onProgress: (progress, status) {
          // Map API progress to 10%-90% of total progress
          final adjustedProgress = 0.1 + (progress * 0.8);
          onProgress(adjustedProgress, status);
        },
      );

      onProgress(0.9, 'Lagrer data...');
      print('✅ Successfully fetched data from API');
      ...
      if (data != null) {
        ...
        onProgress(1.0, 'Fullført!');
        print(
            '✅ Completed downloadAndCacheContentOnlyWithProgress for $publicationId');
      } else {
        throw Exception('No data received from API');
      }
    } catch (e) {
  ...
  // Last ned bilder for en allerede cachet publikasjon med progress callback
  Future<void> downloadImagesForCachedPublicationWithProgress(
    String publicationId, {
    required Function(double progress, String status, int current, int total)
        onProgress,
  }) async {
    try {
      print(
          '🚀 STARTING downloadImagesForCachedPublicationWithProgress for ID: $publicationId');

      onProgress(0.0, 'Sjekker cachet innhold...', 0, 0);
      ...
      if (data != null) {
        onProgress(0.1, 'Analyserer bilder...', 0, 0);
        print('🔍 Found cached content, analyzing image content...');
        _debugHtmlContent(data);

        // Extract image URLs from cached data
        final originalImageUrls = _extractImageUrls(data);
        final totalImages = originalImageUrls.length;
        print('📸 Found $totalImages images to download');

        onProgress(0.15, 'Starter nedlastning...', 0, totalImages);
        ...
        for (int i = 0; i < originalImageUrls.length; i++) {
          ...
          try {
            print('📥 Downloading image ${i + 1}/$totalImages: $fixedUrl');

            // Calculate progress (15% to 85% for downloads)
            final downloadProgress = 0.15 + ((i / totalImages) * 0.7);
            onProgress(downloadProgress, 'Laster ned bilde ${i + 1}...', i + 1,
                totalImages);
        ...
        onProgress(0.85, 'Oppdaterer innhold...', totalImages, totalImages);
        ...
        onProgress(1.0, 'Fullført!', totalImages, totalImages);
        print(
            '✅ Downloaded $downloadedImages images and updated cached content for $publicationId');
      } else {
        throw Exception('No cached content data found');
      }
    } catch (e) {
  ...
  // Fetch full publication from API with progress tracking
  Future<Map<String, dynamic>?> fetchFullPublicationFromApiWithProgress(
    String publicationId, {
    required Function(double progress, String status) onProgress,
  }) async {
    final urls = [
    ...
    for (int urlIndex = 0; urlIndex < urls.length; urlIndex++) {
      ...
      try {
        onProgress(
            0.1 + (urlIndex * 0.1), 'Prøver API endpoint ${urlIndex + 1}...');
        print('Trying publication API URL: $url');

        dynamic data;
        if (url.startsWith('https://')) {
          // Use custom HTTP client for HTTPS URLs with progress
          final request = await _httpClient.getUrl(Uri.parse(url));
          request.headers.set('Content-Type', 'application/json');

          onProgress(0.3 + (urlIndex * 0.1), 'Henter data...');
          final httpResponse =
              await request.close().timeout(const Duration(seconds: 30));

          if (httpResponse.statusCode == 200) {
            onProgress(0.5 + (urlIndex * 0.1), 'Leser respons...');

            // Get expected content length from headers for progress tracking
            ...
            await for (final chunk in httpResponse.transform(utf8.decoder)) {
              ...
              if (contentLength > 0) {
                final downloadProgress = receivedBytes / contentLength;
                onProgress(0.5 + (urlIndex * 0.1) + (downloadProgress * 0.3),
                    'Laster ned data...');
              }
            }

            data = json.decode(responseBody.toString());
            onProgress(0.9, 'Behandler data...');
            print('Success fetching publication data from: $url');
          } else {
            continue;
          }
          ...
          if (response.statusCode == 200) {
            onProgress(0.7 + (urlIndex * 0.1), 'Behandler data...');
            data = json.decode(response.body);
            print('Success fetching publication data from: $url');
          } else {
            continue;
  ...
  // Fetch all items (chapters and subchapters) from publication data
  Future<List<Map<String, dynamic>>> fetchAllPublicationItems(
      String publicationId) async {
    try {
      final fullContent = await getCachedFullContent(publicationId);
      if (fullContent == null) {
        return [];
      }
      ...
      print('🔍 FullContent keys: ${fullContent.keys.toList()}');

      // Publication Details should show root-level objects as clickable links
      // Look for root-level array or individual objects with Title and Subchapters
      List<dynamic> rootObjects = [];

      // Look for arrays at the root level that contain objects with Title and Subchapters
      for (var key in fullContent.keys) {
        var value = fullContent[key];
        if (value is List && value.isNotEmpty) {
          // Check if this array contains objects with Title/Name and Subchapters
          var firstItem = value.first;
          if (firstItem is Map &&
              (firstItem.containsKey('Title') ||
                  firstItem.containsKey('Name')) &&
              (firstItem.containsKey('Subchapters') ||
                  firstItem.containsKey('SubChapters'))) {
            rootObjects = value;
            print(
                '📖 Found root objects in key "$key": ${rootObjects.length} items');
            break;
          }
        }
      }

      // If no array found, treat the fullContent itself as a single root object if it has the right structure
      if (rootObjects.isEmpty &&
          (fullContent.containsKey('Title') ||
              fullContent.containsKey('Name')) &&
          (fullContent.containsKey('Subchapters') ||
              fullContent.containsKey('SubChapters'))) {
        rootObjects = [fullContent];
        print('📖 Found single root object in fullContent');
      }

      print('📋 Total root objects found: ${rootObjects.length}');

      // Process root objects - these will be shown in Publication Details
      for (var rootObject in rootObjects) {
        if (rootObject is Map<String, dynamic>) {
          final title =
              rootObject['Title'] ?? rootObject['Name'] ?? 'Uten tittel';
          print('📖 Processing root object: $title');

          // Add root object as a chapter item (for Publication Details view)
          allItems.add({
            'type': 'chapter',
            'title': title,
            'id': rootObject['Id'] ?? rootObject['id'] ?? '',
            'data': rootObject,
          });
        }
      }
      ...
      print(
          '📋 Found ${allItems.length} total items (chapters + subchapters) in publication $publicationId');
      return allItems;
    } catch (e) {
      print('❌ Error fetching publication items: $e');
      return [];
    }
  }
